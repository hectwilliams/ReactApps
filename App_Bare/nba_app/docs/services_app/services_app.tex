...
------------------+-------+----------------+----------+------------+------------+--------+-----------------------------------------
 ...
 1/1/2012 20:00   |   3.2 |            1.3 |       87 |         19 |         25 |   99.5 | Cloudy
    ...
    A tensorflow built sequence-to-vector network was exported into Binny. Clients can infer the network/model using \small\textit{ HTTP POST} requests. The following steps were used to create and load model.
    \begin{itemize}

        \item Create virtual environment using python 3.10   \smallskip
        \item Install package housing command-line utility tensorflowjs\_converter : 
        \begin{verbatim} 
pip install --upgrade tensorflowjs  \smallskip
     \end{verbatim} 
        \item Train temperaure data using simple DNN of univariate sets  \smallskip
            
        \item Export model (tensorflow formatted model) 
        \begin{itemize}
            
            \item \smallskip

            \begin{verbatim} 
tensorflowjs_converter  --input_format=tf_saved_model --output_format=tfjs_graph_model $PWD/saved_model_dir  $PWD/web_model_gen_dir
ensorflowjs_converter --input_format=tf_saved_model --output_format=tfjs_graph_model $PWD/saved_model $PWD/web_model             
\end{verbatim} 
            \item Import model into Binny webserver \smallskip
            \item Validate loaded model  \smallskip
        \end{itemize} 
    \end{itemize} 
